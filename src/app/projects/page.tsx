    ...
    <>
      ...
      <div className="fixed top-1 w-full pr-44 z-20 bg-white flex justify-between items-center mb-4">
        ...
        <div className="relative">
          <Search
            className="absolute left-2 text-gray-500 top-3"
            size={20}
          />
          <input
      ...
      {/* Table headers */}
      <div className="mt-8">
        <div className="grid grid-cols-[500px_120px_120px_150px_200px] gap-4 p-3 rounded-xl bg-[#f7f7fa] font-bold">
          ...
          <div className="min-w-[200px]">Venue</div>
        </div>
          ...
          {filteredProjects
            ...
            .map((project) => (
              <Link key={project.id} href={`/projects/${project.id}`}>
                <div className="grid grid-cols-[500px_120px_120px_150px_200px] gap-4 p-3 border-b items-center hover:bg-gray-50 transition">
                  <div className="break-words min-w-[200px]">{project.name}</div>
                  <div className="min-w-[120px]">{project.startDate}</div>
                  <div className="min-w-[120px]">{project.endDate}</div>
                  <div
                    className={`inline-flex items-center justify-center p-1 rounded-full ring-2 ring-inset min-w-[150px]
                      ${
                        project.status === "Design Submitted"
                          ? "ring-green-400 text-green-400 bg-green-50"
                          : project.status === "Project Confirmed" ||
                            project.status === "Admin Approved"
                          ? "ring-yellow-200 text-yellow-400 bg-yellow-50"
                          : "ring-red-500 text-red-400 bg-red-50"
                      }`}
                  >
                    {project.status}
                  </div>
                  <div className="min-w-[200px]">{project.venue}</div>
                </div>
              </Link>
            ))}
        </div>
